...
export class KnowledgePanel {
  ...
  update(scene) {
    ...
    if (!description) {
      this.contentContainer.innerHTML = '<p class="placeholder-text">暂无该场景的详细物理原理解析。</p>';
      return;
    }
  ...
  renderMath() {
    ...
    mathElements.forEach(el => {
      try {
        const tex = el.textContent || el.getAttribute('data-tex');
        const displayMode = el.tagName.toLowerCase() === 'div' || el.classList.contains('display');
