...
export class RealTimeChart {
   ...
   */
  constructor(canvasId, label, colors = THEME.colors.chart.series, capacity = 3600) {
  ...
  draw() {
    ...
    if (newestT <= windowSec) {
      ...
      xToPix = t => t * (width / windowSec);
      tStart = 0;
      tEnd = windowSec;
    } else {
      // 2. 滚动阶段 (t > windowSec): X 轴随时间向左滚动
      xToPix = t => width - (newestT - t) * (width / windowSec);
    ...
    // Y轴坐标转换
    const yToPix = y => height / 2 - (y / yMax) * (height * 0.45);
