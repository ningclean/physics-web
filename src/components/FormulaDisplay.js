...
export class FormulaDisplay {
  ...
  /**
   * 渲染 LaTeX 公式字符串。
   * @param {string|string[]} latex - 要渲染的 LaTeX 字符串或字符串数组
   * @param {boolean} displayMode - 是否以显示模式渲染（居中块）
   */
  setFormula(latex, displayMode = true) {
    ...
    formulas.forEach(item => {
      ...
      if (typeof item === 'object' && item !== null && item.tex) {
        if (item.label) {
      ...
      // 渲染参数说明
      if (typeof item === 'object' && item.params && Array.isArray(item.params)) {
