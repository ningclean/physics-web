...
export class ControlPanel extends EventBus {
   ...
   * ]
   * @param {Array} legend - 图例配置数组 (可选)
   * @param {string} sceneKey - 场景的唯一标识符 (可选)
   */
  setup(params, config, legend = [], sceneKey = 'scene') {
    this.clear();
  ...
  async recordGif(duration, btn) {
    ...
    console.log(`Starting GIF recording: ${duration.toFixed(0)}ms, ${targetFrames} frames, interval ${captureInterval.toFixed(0)}ms`);
  ...
  createRangeControl(parent, params, config) {
  ...
  createBooleanControl(parent, params, config) {
    const label = document.createElement('label');
    label.style.fontSize = '0.9rem';
  ...
  createColorControl(parent, params, config) {
    const label = document.createElement('label');
    label.textContent = `${config.label}: `;
    label.style.fontSize = '0.9rem';
    label.style.minWidth = '120px';
  ...
  createSelectControl(parent, params, config) {
    const label = document.createElement('label');
    label.textContent = `${config.label}: `;
    label.style.fontSize = '0.9rem';
    label.style.minWidth = '120px';
  ...
  createActionControl(parent, config) {
