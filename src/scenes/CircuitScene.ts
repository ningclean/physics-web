...
/**
 * Circuit scene parameters interface
 */
interface CircuitParams {
  voltage: number;
  r1: number;
  r2: number;
  circuitType: 'series' | 'parallel' | 'mixed' | 'voltage' | 'current';
  showCurrent: boolean;
  currentSpeed: number;
  switch1: boolean;
  switch2: boolean;
...
/**
 * Hit region interface for interactive elements
 */
interface HitRegion {
  id: string;
  x: number;
...
/**
 * Point interface for wire paths
 */
interface Point {
  x: number;
  y: number;
...
export class CircuitScene extends Scene {
  ...
  registerHitRegion(id: string, x: number, y: number, w: number, h: number): void {
    ...
    return [
      {
        ...
        options: [
          ...
          { label: '电流测量电路 (Current)', value: 'current' },
        ],
  ...
  render(ctx: CanvasRenderingContext2D): void {
    ...
    if (S1) {
      ...
      if (this.params.circuitType === 'series') {
        ...
        // 电压测量电路：串联 Battery -> S1 -> L1 -> L2 -> GND
        const R_total = R1 + R2;
        I_total = V / R_total;
        I1 = I_total;
        I2 = I_total;
      } else if (this.params.circuitType === 'current') {
        // 电流测量电路：串联 Battery -> S1 -> L1 -> Ammeter -> L2 -> GND
        const R_total = R1 + R2;
        I_total = V / R_total;
        I1 = I_total;
        I2 = I_total;
  ...
  // 渲染串联电路
  renderSeries(ctx: CanvasRenderingContext2D, leftX: number, rightX: number, topY: number, bottomY: number, s1X: number, R1: number, R2: number, S2: boolean, P1: number, P2: number, I: number, cy: number): void {
  ...
  // 渲染电压测量电路（电压表并联）
  renderVoltageMeasurement(ctx: CanvasRenderingContext2D, leftX: number, rightX: number, topY: number, bottomY: number, s1X: number, R1: number, R2: number, P1: number, P2: number, I_total: number, cy: number): void {
    // 结构: Battery -> S1 -> L1 -> L2 -> GND
    // 电压表并联在 L2 两端
    // Manhattan routing，红色导线，无节点
    const l1X = (leftX + rightX) / 2 - 80;
    const l2X = (leftX + rightX) / 2 + 80;
    const vMeterY = topY - 100; // 移得更远以避免重叠
    // Battery Top to S1 Left
    drawWire(
      ctx,
      [
  ...
  // 渲染电流测量电路（安培计串联）
  renderCurrentMeasurement(ctx: CanvasRenderingContext2D, leftX: number, rightX: number, topY: number, bottomY: number, s1X: number, R1: number, R2: number, P1: number, P2: number, I_total: number, cy: number): void {
    // 结构: Battery -> S1 -> L1 -> Ammeter -> L2 -> GND
    // 安培计串联在 L1 和 L2 之间
    // Manhattan routing，红色导线，无节点
    const l1X = (leftX + rightX) / 2 - 80;
    ...
    // L1 Right to Ammeter Left
    drawWire(
      ctx,
      [
        { x: l1X + 6, y: topY },
        { x: ammeterX - 30, y: topY },
      ],
      '#d32f2f'
    );
    // Ammeter Right to L2 Left
    drawWire(
      ctx,
      [
        { x: ammeterX + 30, y: topY },
    ...
    ctx.fillText('S1', s1X, topY - 30);
    // 安培计
    drawMultimeter(ctx, ammeterX, topY, 60, I_total.toFixed(2), 'A');
    ctx.fillText('A', ammeterX, topY + 40);
    // 动画（电流）
    if (this.params.showCurrent && I_total > 0.01) {
      const path = [
        { x: leftX, y: cy - 40 },
  ...
  // 渲染并联电路
  renderParallel(ctx: CanvasRenderingContext2D, leftX: number, rightX: number, topY: number, bottomY: number, s1X: number, R1: number, R2: number, S2: boolean, P1: number, P2: number, I1: number, I2: number, I_total: number, cy: number): void {
    // 布局:
  ...
  // 渲染混合电路
  renderMixed(ctx: CanvasRenderingContext2D, leftX: number, rightX: number, topY: number, bottomY: number, s1X: number, R1: number, R2: number, S2: boolean, P1: number, P2: number, I1: number, I2: number, I_total: number, cy: number): void {
  ...
  getBulbColor(power: number): string {
  ...
  animateCurrent(ctx: CanvasRenderingContext2D, points: Point[], current: number): void {
