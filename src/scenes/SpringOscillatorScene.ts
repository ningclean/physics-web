...
import { drawDot, drawVector } from '../utils/draw.ts';
import { drawSpring } from '../utils/graphics.ts';
...
interface SpringOscillatorParams {
...
export class SpringOscillatorScene extends Scene {
  ...
  update(dt: number, t: number): void {
    ...
    // 定义受力函数 (用于积分器)
    const forceFn = (pos: number, vel: number, t: number) => {
      // F_spring = -k * x
    ...
    // 定义加速度函数 (用于半隐式欧拉)
    const accelerationFunc = (pos: number, vel: number, t: number) => {
    ...
    const newState = Integrator.semiImplicitEuler(
      this.phys.x,
      this.phys.v,
      t,
      dt,
  ...
  renderHorizontal(ctx: CanvasRenderingContext2D, isLightBg: boolean, mainColor: string): void {
    const { width, height } = this;
    ...
    if (showFriction) {
      ...
      drawVector(
        ctx,
        ballX,
        ballY + ballRadius + offset,
        F_damping * fScale,
        0,
        THEME.colors.vectors.damping,
        'F_d'
      );
    }
  }

  renderVertical(ctx: CanvasRenderingContext2D, isLightBg: boolean, mainColor: string): void {
    const { width, height } = this;
