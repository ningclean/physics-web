...
export class FreeFallScene extends Scene {
  ...
  getControlConfig() {
    return [
      { 
        type: 'range', key: 'height', label: '初始高度 (H)', min: 50, max: 400, step: 10,
        description: '小球起始距离地面的高度 (px)',
        onChange: () => this.resetSimulation()
      },
      { 
        type: 'range', key: 'gravity', label: '重力加速度 (g)', min: 1, max: 30, step: 0.1,
        description: '环境重力大小',
        onChange: (val) => { this.phys.simG = val * 10; }
      },
      { 
        type: 'range', key: 'v0', label: '初速度 (v0)', min: -20, max: 20, step: 1,
        description: '垂直方向初速度 (负值向上，正值向下)',
        onChange: () => this.resetSimulation()
      },
      { 
        type: 'range', key: 'restitution', label: '弹性系数 (e)', min: 0, max: 1, step: 0.1,
