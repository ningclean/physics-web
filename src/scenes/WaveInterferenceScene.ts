...
import { drawDot } from '../utils/draw.ts';
 ...
 * 波干涉场景参数接口
 */
interface WaveInterferenceParams {
  f1: number; // Hz
  f2: number; // Hz
  d: number; // px (波源间距)
  v: number; // px/s (波速)
...
export class WaveInterferenceScene extends Scene {
  ...
  render(ctx: CanvasRenderingContext2D): void {
    ...
    const src1: WaveSource = { x: cx - dScaled / 2, y: cy };
    const src2: WaveSource = { x: cx + dScaled / 2, y: cy };
