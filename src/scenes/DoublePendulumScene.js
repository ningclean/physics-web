...
import { drawDot, drawLine } from '../utils/draw.js';
...
export class DoublePendulumScene extends Scene {
  ...
  getControlConfig() {
    return [
      { 
        type: 'range', key: 'm1', label: '质量 1 (m1)', min: 0.5, max: 5.0, step: 0.1,
        description: '上方摆球的质量 (kg)。', resetOnChange: false
      },
      { 
        type: 'range', key: 'm2', label: '质量 2 (m2)', min: 0.5, max: 5.0, step: 0.1,
        description: '下方摆球的质量 (kg)。', resetOnChange: false
      },
      { 
        type: 'range', key: 'L1', label: '摆长 1 (L1)', min: 0.5, max: 2.0, step: 0.1,
        description: '上方摆杆长度 (m)。', resetOnChange: false
      },
      { 
        type: 'range', key: 'L2', label: '摆长 2 (L2)', min: 0.5, max: 2.0, step: 0.1,
        description: '下方摆杆长度 (m)。', resetOnChange: false
      },
      { 
        type: 'range', key: 'theta1_0', label: '初始角 1', min: -180, max: 180, step: 5,
        description: '上方摆杆初始角度 (度)。'
      },
      { 
        type: 'range', key: 'theta2_0', label: '初始角 2', min: -180, max: 180, step: 5,
      ...
      { 
        type: 'range', key: 'trailLength', label: '轨迹长度', min: 100, max: 2000, step: 100,
        description: '保留的轨迹点数量。', resetOnChange: false
      },
      {
  ...
  update(dt, t) {
    ...
    const derivatives = (state, time) => {
      ...
      const { alpha1, alpha2 } = Physics.doublePendulumAccelerations(
          { m1, m2, L1, L2, g }, 
          state
      );
      return {
