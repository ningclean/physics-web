...
import { drawAxes, drawCircle, drawDot, drawVector, drawTrail } from '../utils/draw.js';
...
export class CircularScene extends Scene {
  ...
  getControlConfig() {
    return [
      { 
        type: 'range', key: 'omega', label: '角速度 (ω)', min: 0.1, max: 5, step: 0.1,
        description: '控制物体绕圆心转动的快慢，值越大转得越快。'
      },
      { 
        type: 'range', key: 'radius', label: '半径 (R)', min: 50, max: 250, step: 10,
  ...
  render(ctx, alpha = 1.0) {
    ...
    if (this.params.showVel) {
      ...
      drawVector(ctx, st.x, st.y, (st.vx / speed) * scale, (st.vy / speed) * scale, THEME.colors.vectors.velocity, 'v');
    }

    if (this.params.showAcc) {
      const accel = Math.hypot(st.ax, st.ay) || 1;
      const scale = Math.min(0.15 * st.radius, 50) * (accel ? 1 : 0) * vScale;
      drawVector(ctx, st.x, st.y, (st.ax / accel) * scale, (st.ay / accel) * scale, THEME.colors.vectors.acceleration, 'a');
    }
  }
}
