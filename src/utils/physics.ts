...
interface DoublePendulumAccelerations {
  alpha1: number;
  alpha2: number;
}
...
export const Physics = {
   ...
   * @returns {number} 弹力
   */
  springForce(k: number, displacement: number): number {
   ...
   * @returns {number} 阻尼力
   */
  dampingForce(c: number, velocity: number): number {
    return -c * velocity;
  },
   ...
   * @returns {number} 周期
   */
  pendulumPeriod(length: number, g: number): number {
   ...
   * @returns {number} 角频率
   */
  springNaturalFrequency(k: number, m: number): number {
   ...
   * @returns {number} 伸长量
   */
  springStaticExtension(m: number, g: number, k: number): number {
    return (m * g) / k;
  },
   ...
   */
  pendulumTension(m: number, g: number, theta: number, L: number, omega: number): number {
   ...
   * @returns {number} 引力大小
   */
  gravitationalForce(G: number, M: number, m: number, r: number): number {
