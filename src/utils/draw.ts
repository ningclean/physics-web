...
export function drawAxes(ctx: CanvasRenderingContext2D, width: number, height: number, color: string = '#444'): void {
  ctx.save();
...
export function drawCircle(ctx: CanvasRenderingContext2D, cx: number, cy: number, r: number, color: string = THEME.colors.objects.referenceLine): void {
  ctx.save();
  ctx.strokeStyle = color;
  ctx.lineWidth = THEME.sizes.lineWidth.thin;
  ctx.beginPath();
...
export function drawDot(
  ...
  x: number,
  y: number,
  color: string = THEME.colors.objects.ball.red,
  radius: number = THEME.sizes.dotRadius
): void {
  ctx.save();
  ctx.fillStyle = color;
  ctx.beginPath();
 ...
 */
export function drawVector(ctx: CanvasRenderingContext2D, fromX: number, fromY: number, dx: number, dy: number, color: string, label?: string): void {
  ctx.save();
  ctx.strokeStyle = color;
  ctx.fillStyle = color;
  ...
  if (label) {
    ...
    ctx.fillText(
      label,
      fromX + dx + (dx >= 0 ? padding : -padding),
      fromY + dy + (dy >= 0 ? padding : -padding)
    );
  }
  ctx.restore();
}

export function drawTrail(ctx: CanvasRenderingContext2D, points: Point[], color: string = '#ffffff'): void {
...
export function drawLine(ctx: CanvasRenderingContext2D, x1: number, y1: number, x2: number, y2: number, color: string = '#fff', lineWidth: number = 1): void {
  ctx.save();
  ctx.strokeStyle = color;
