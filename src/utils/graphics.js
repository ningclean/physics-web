 ...
 */
export function drawStickFigure(ctx, x, footY, headY, color, label, angle = 0) {
 ...
 */
export function drawBall(ctx, x, y, r, color) {
    ctx.save();
    ctx.beginPath();
 ...
 */
export function drawBlock(ctx, x, y, w, h, color, text, angle = 0) {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(angle);
 ...
 */
export function drawGround(ctx, x1, y1, x2, y2, thickness = 10) {
    ctx.save();
    ctx.strokeStyle = '#888';
    ctx.lineWidth = 2;
 ...
 */
export function drawSpring(ctx, x1, y1, x2, y2, coils, width, color, lineWidth = 2) {
    const dx = x2 - x1;
    const dy = y2 - y1;
    const len = Math.hypot(dx, dy);
 ...
 * @param {string} color 颜色
 */
export function drawDamper(ctx, x1, y1, x2, y2, width, color) {
    const dx = x2 - x1;
    const dy = y2 - y1;
    const len = Math.hypot(dx, dy);
 ...
 */
export function drawSupport(ctx, x, y, size, color, angle = 0) {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(angle);
 ...
 * @param {number} angle 旋转角度
 */
export function drawCart(ctx, x, y, w, h, r, color, angle = 0) {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(angle);
    ...
    // 轮轴点
    ctx.fillStyle = '#fff';
    ctx.beginPath(); ctx.arc(-wheelOffset, wheelY, 2, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.arc(wheelOffset, wheelY, 2, 0, Math.PI*2); ctx.fill();
 ...
 * @param {string} color 颜色
 */
export function drawPulley(ctx, x, y, r, color) {
    ctx.save();
    ctx.translate(x, y);
 ...
 */
export function drawRuler(ctx, x, y, length, angle = 0, unit = 'cm') {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(angle);
 ...
 * @param {number} r 半径
 */
export function drawProtractor(ctx, x, y, r) {
    ctx.save();
    ctx.translate(x, y);
 ...
 */
export function drawStopwatch(ctx, x, y, r, time) {
    ctx.save();
    ctx.translate(x, y);
 ...
 * @param {number} angle 旋转角度
 */
export function drawMagnet(ctx, x, y, w, h, angle = 0) {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(angle);
 ...
 */
export function drawCharge(ctx, x, y, r, charge) {
    ctx.save();
    ctx.translate(x, y);
 ...
 * @param {number} angle 旋转角度
 */
export function drawBattery(ctx, x, y, w, h, angle = 0) {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(angle);
 ...
 */
export function drawLightBulb(ctx, x, y, size, isOn = false, color = '#ffff00') {
    ctx.save();
    ctx.translate(x, y);
 ...
 * @param {number} lineWidth 线宽
 */
export function drawWire(ctx, points, color = '#d32f2f', lineWidth = 3) {
 ...
 */
export function drawSwitch(ctx, x, y, width, isOpen = true) {
    ctx.save();
    ctx.translate(x, y);
    ...
    ctx.fillStyle = '#333';
    // 左
    ctx.beginPath(); ctx.arc(-width/3, 0, r, 0, Math.PI*2); ctx.fill();
    // 右
    ctx.beginPath(); ctx.arc(width/3, 0, r, 0, Math.PI*2); ctx.fill();
 ...
 */
export function drawMultimeter(ctx, x, y, width, value, unit) {
 ...
 */
export function drawResistor(ctx, x, y, width, bands = ['#8d6e63', '#000000', '#d32f2f', '#ffd700']) {
    ctx.save();
    ctx.translate(x, y);
 ...
 */
export function drawEye(ctx, x, y, size, angle = 0) {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(angle);
 ...
 */
export function drawFish(ctx, x, y, color, opacity = 1.0) {
    ctx.save();
    ctx.translate(x, y);
